...
def post_process_naacl(labels, preds):
    targs_proc = []
    preds_proc = []

    for idx, label in enumerate(labels):
        for k in label.keys():
            targs_proc.append(label[k])
            # TODO: replace magic number with code
            pred_cls = 1 if preds[idx][int(k)] == 4 else 0
            preds_proc.append(pred_cls)

    return targs_proc, preds_proc
