...
def load_init_det_model(model_module, config):
    """
    Initialize a model and load a checkpoint if so desired (if the checkpoint is available.)
    ...
    model = model_module(config).to(config.DEVICE)
    optimizer = torch.optim.Adam(model.parameters(), lr=config.LEARNING_RATE)
    save_path = config.PATH_TO_CHECKPOINT_DET.format(config.LOAD_CHECKPOINT_TYPE)
    save_path += 'idiomDetectSeq2Seq.mdl'
    ...
    if os.path.exists(save_path) and config.CONTINUE_TRAIN:
        ...
        print('=> No checkpoint found or NOT continue train! Train from scratch!')
    model.eval()
