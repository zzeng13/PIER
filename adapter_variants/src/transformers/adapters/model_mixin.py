...
from .composition import AdapterCompositionBlock, Fuse, Flow, Stack, parse_composition
from .configuration import (
    ADAPTERFUSION_CONFIG_MAP,
    DEFAULT_ADAPTERFUSION_CONFIG,
    ADAPTERFLOW_CONFIG_MAP,
    DEFAULT_ADAPTERFLOW_CONFIG,
    AdapterConfig,
    AdapterFusionConfig,
    AdapterFlowConfig,
    ModelAdaptersConfig,
...
from .loading import AdapterFusionLoader, AdapterFlowLoader, AdapterLoader, PredictionHeadLoader, WeightsLoader
...
class ModelAdaptersMixin(PushAdapterToHubMixin, ABC):
    ...
    def _init_adapter_modules(self):
        ...
        if hasattr(self.config, "adapter_flow_models"):
            for flow_adapter_names in self.config.adapter_flow_models:
                self._add_flow_layer(flow_adapter_names)
    ...
    def train_fusion(self, adapter_setup: Union[list, AdapterCompositionBlock], unfreeze_adapters=False):
        ...
        warnings.warn(
            "traom_fusion() has been deprecated in favor of train_adapter_fusion(). Please use the newer method instead.",
            FutureWarning,
        )
        self.train_adapter_fusion(adapter_setup, unfreeze_adapters=unfreeze_adapters)

    def train_flow(self, adapter_setup: Union[list, AdapterCompositionBlock], unfreeze_adapters=False):
        """Sets the model into mode for training of adapter flow determined by a list of adapter names."""
        warnings.warn(
            "train_flow() has been deprecated in favor of train_adapter_flow(). Please use the newer method instead.",
            FutureWarning,
        )
        self.train_adapter_flow(adapter_setup, unfreeze_adapters=unfreeze_adapters)
    ...
    @abstractmethod
    def train_adapter_flow(self, adapter_setup: Union[list, AdapterCompositionBlock], unfreeze_adapters=False):
        """Sets the model into mode for training of adapter flow determined by a list of adapter names."""
        pass
    ...
    @abstractmethod
    def _add_flow_layer(self, adapter_names):
        pass
    ...
    def set_adapter_flow_config(self, adapter_flow_config, override_kwargs=None):
        """
        Sets the adapter flow configuration.

        Args:
            adapter_flow_config (str or dict): adapter flow configuration, can be either:

                - a string identifying a pre-defined adapter flow configuration
                - a dictionary representing the adapter flow configuration
                - the path to a file containing the adapter flow configuration
        """
        if override_kwargs is None:
            override_kwargs = {}
        if isinstance(adapter_flow_config, str) and adapter_flow_config in ADAPTERFLOW_CONFIG_MAP:
            self.config.adapter_flow = AdapterFlowConfig.load(adapter_flow_config, **override_kwargs)
        elif isinstance(adapter_flow_config, Mapping):
            self.config.adapter_flow = adapter_flow_config
        else:
            raise ValueError("Invalid adapter type {}".format(adapter_flow_config))
    ...
    def add_flow(self, adapter_names: Union[Flow, list], adapter_flow_config=None, override_kwargs=None):
        warnings.warn(
            "add_flow() has been deprecated in favor of add_adapter_flow(). Please use the newer method instead.",
            FutureWarning,
        )
        self.add_adapter_flow(adapter_names, adapter_flow_config, override_kwargs)

    def add_adapter_flow(self, adapter_names: Union[Flow, list], adapter_flow_config=None, override_kwargs=None):
        """
        Adds AdapterFlow to the model with alll the necessary configurations and weight initializations
        ...
        Args:
            ...
            adapter_flow_config (str or dict): adapter flow configuration, can be either:
                ...
                - a dictionary representing the adapter flow configuration
                - the path to a file containing the adapter flow configuration
            override_kwargs: dictionary items for values which should be overwritten in the default AdapterFlow configuration
        """
        # TODO-V2 Allow nested items or directly pass Fuse block?
        if isinstance(adapter_names, Flow):
            adapter_names = adapter_names.children
        if not hasattr(self.config, "adapter_flow"):
            if override_kwargs is None:
                override_kwargs = {}
            if adapter_flow_config is not None:
                self.set_adapter_flow_config(adapter_flow_config, **override_kwargs)
            else:
                self.set_adapter_flow_config(DEFAULT_ADAPTERFLOW_CONFIG)
        elif hasattr(self.config, "adapter_flow") and adapter_flow_config is not None:
            # This behavior may be a bit unintuitive as the given argument is ignored, but we can't throw an error because of the loader.
            logger.warning("An AdapterFlow config has already been set and will NOT be overwritten")

        if not hasattr(self.config, "adapter_flow_models"):
            self.config.adapter_flow_models = []
        if isinstance(adapter_names, list):
            adapter_flow_name = ",".join(adapter_names)
        else:
            adapter_flow_name = adapter_names
        if adapter_flow_name not in self.config.adapter_flow_models:
            self.config.adapter_flow_models.append(adapter_flow_name)
            self.base_model._add_flow_layer(adapter_names)
    ...
    def delete_adapter_flow(self, adapter_names: Union[Flow, list]):
        """
        Deletes the AdapterFlow layer of the specified adapters.

        Args:
            adapter_names (Union[Flow, list]): List of adapters for which to delete the AdapterFlow layer.
        """
        if isinstance(adapter_names, Flow):
            adapter_flow_name = ",".join(adapter_names.children)
        elif isinstance(adapter_names, list):
            adapter_flow_name = ",".join(adapter_names)
        else:
            adapter_flow_name = adapter_names
        ...
        if (
            not hasattr(self.config, "adapter_flow_models")
            or adapter_flow_name not in self.config.adapter_flow_models
        ):
            logger.info("No AdapterFlow '%s' found for deletion. Skipping.", adapter_flow_name)
            return
        self.config.adapter_flow_models.remove(adapter_flow_name)
        self.base_model._delete_flow_layer(adapter_flow_name)
    ...
    def load_adapter_flow(
        self,
        adapter_flow_name_or_path: str,
        load_as: str = None,
        custom_weights_loaders: Optional[List[WeightsLoader]] = None,
        **kwargs
    ) -> str:
        ...
        Args:
            adapter_flow_name_or_path (str): can be either:

                - the identifier of a pre-trained task adapter flow module to be loaded from Adapter Hub
                - a path to a directory containing adapter weights saved using `model.saved_adapter()`
                - a URL pointing to a zip folder containing a saved adapter module
            config (dict or str, optional): The requested configuration of the adapter flow.
                If not specified, will be either: - the default adapter config for the requested adapter flow if
                specified - the global default adapter flow config
            model_name (str, optional): The string identifier of the pre-trained model.
            load_as (str, optional): Load the adapter using this name. By default, the name with which the adapter was
                    saved will be used.
        ...
        loader = AdapterFlowLoader(self)
        load_dir, load_name = loader.load(adapter_flow_name_or_path, load_as)
        # load additional custom weights
        if custom_weights_loaders:
            for weights_loader in custom_weights_loaders:
                weights_loader.load(
    ...
    def save_all_adapter_flows(
        ...
        if not hasattr(self.config, "adapter_flow_models"):
            return
        for name in self.config.adapter_flow_models:
            adapter_flow_config = self.config.adapter_flow
            h = get_adapter_config_hash(adapter_flow_config)
            save_path = join(save_directory, name)
            if meta_dict:
                meta_dict.update({"config_id": h})
            else:
                meta_dict = {"config_id": h}
            self.save_adapter_flow(save_path, name, custom_weights_loaders=custom_weights_loaders)
...
class ModelWithHeadsAdaptersMixin(ModelAdaptersMixin):
    ...
    def train_adapter_flow(self, adapter_setup: Union[list, AdapterCompositionBlock], unfreeze_adapters=False):
        """Sets the model into mode for training of adapter flow determined by a list of adapter names."""
        self.base_model.train_adapter_flow(adapter_setup, unfreeze_adapters=unfreeze_adapters)
