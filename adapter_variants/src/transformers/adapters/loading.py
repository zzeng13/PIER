...
from .utils import (
    ...
    ADAPTERFLOW_CONFIG_NAME,
    ADAPTERFLOW_WEIGHTS_NAME,
    CONFIG_NAME,
    HEAD_CONFIG_NAME,
    HEAD_WEIGHTS_NAME,
    WEIGHTS_NAME,
...
class AdapterFlowLoader(WeightsLoader):
    """
    ...
    def __init__(self, model, error_on_missing=True):
        super().__init__(model, ADAPTERFLOW_WEIGHTS_NAME, ADAPTERFLOW_CONFIG_NAME)
        self.error_on_missing = error_on_missing

    def filter_func(self, adapter_flow_name):
        return lambda x: "adapter_flow_layer.{}".format(adapter_flow_name) in x

    def rename_func(self, old_name, new_name):
        return lambda k: k.replace(
            "adapter_flow_layer.{}".format(old_name), "adapter_flow_layer.{}".format(new_name)
        )

    def save(self, save_directory: str, name: str):
        """
        Saves a AdapterFlow module into the given directory.

        Args:
            save_directory (str): The directory to save the weights in.
            name (str, optional): The AdapterFlow name.
        """

        if hasattr(self.model.config, "adapter_flow_models"):
            if name not in self.model.config.adapter_flow_models:
                if self.error_on_missing:
                    raise ValueError(f"Unknown AdapterFlow '{name}'.")
                else:
                    logger.debug(f"No AdapterFlow with name '{name}' available.")
                    return
        ...
        adapter_flow_config = self.model.config.adapter_flow

        # Save the adapter flow configuration
        config_dict = build_full_config(
            adapter_flow_config,
            self.model.config,
            name=name,
    ...
    def load(self, save_directory, load_as=None, loading_info=None):
        """
        Loads a AdapterFlow module from the given directory.
        ...
        Returns:
            ...
            and the name of the loaded weights.
        """
        if not exists(join(save_directory, ADAPTERFLOW_WEIGHTS_NAME)):
            if self.error_on_missing:
                raise ValueError("Loading path should be a directory where AdapterFlow is saved.")
            else:
                logger.debug("No matching adapter flow found in '{}'".format(save_directory))
                return None, None

        config = self.weights_helper.load_weights_config(save_directory)
        if not hasattr(self.model.config, "adapter_flow_models"):
            self.model.config.adapter_flow_models = []

        adapter_flow_name = load_as or config["name"]
        if adapter_flow_name in self.model.config.adapter_flow_models:
            logger.warning("Overwriting existing adapter flow module '{}'".format(adapter_flow_name))
        self.model.add_adapter_flow(adapter_flow_name, config["config"])

        # Load AdapterFlow weights
        filter_func = self.filter_func(adapter_flow_name)
        if load_as:
            rename_func = self.rename_func(config["name"], load_as)
        else:
            rename_func = None
        ...
        return save_directory, adapter_flow_name
