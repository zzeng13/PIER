...
from .composition import AdapterCompositionBlock, BatchSplit, Fuse, Flow, Parallel, Split, Stack, parse_composition
from .modeling import Adapter, BertFusion, BertFlow
...
class AdapterLayerBaseMixin(ABC):
    ...
    def _init_adapter_modules(self):
        ...
        self.adapter_flow_layer = nn.ModuleDict(dict())
    ...
    def add_flow_layer(self, adapter_names: Union[List, str]):
        """See BertModel.add_flow_layer"""
        adapter_names = adapter_names if isinstance(adapter_names, list) else adapter_names.split(",")
        if self.config.adapters.common_config_value(adapter_names, self.adapter_config_key):
            flow = BertFlow(self.config)
            flow.train(self.training)  # make sure training mode is consistent
            self.adapter_flow_layer[",".join(adapter_names)] = flow

    def delete_flow_layer(self, adapter_names: Union[List, str]):
        adapter_names = adapter_names if isinstance(adapter_names, str) else ",".join(adapter_names)
        if adapter_names in self.adapter_flow_layer:
            del self.adapter_flow_layer[adapter_names]

    def enable_adapters(self, adapter_setup: AdapterCompositionBlock,
                        unfreeze_adapters: bool, unfreeze_fusion: bool):
        """
                ...
                if isinstance(sub_setup, Fuse):
                    if sub_setup.name in self.adapter_fusion_layer:
                        for param in self.adapter_fusion_layer[sub_setup.name].parameters():
                            param.requires_grad = True
        unfreeze_flow = unfreeze_fusion
        if unfreeze_flow:
            if isinstance(adapter_setup, Flow):
                if adapter_setup.name in self.adapter_flow_layer:
                    for param in self.adapter_flow_layer[adapter_setup.name].parameters():
                        param.requires_grad = True
            for sub_setup in adapter_setup:
                if isinstance(sub_setup, Flow):
                    if sub_setup.name in self.adapter_flow_layer:
                        for param in self.adapter_flow_layer[sub_setup.name].parameters():
                            param.requires_grad = True
    ...
    def get_adapter_preparams(
        ...
        if hasattr(self.config, "adapter_flow") and self.config.adapter_flow["query_before_ln"]:
            query = hidden_states
        ...
        if hasattr(self.config, "adapter_flow") and not self.config.adapter_flow["query_before_ln"]:
            query = hidden_states
    ...
    def adapter_stack(self, adapter_setup: Stack, hidden_states, input_tensor, lvl=0):
        ...
        for i, adapter_stack_layer in enumerate(adapter_setup):
            ...
            # Case 0: We have a nested flow layer -> call flow method
            if isinstance(adapter_stack_layer, Flow):
                hidden_states = self.adapter_flow(adapter_stack_layer, hidden_states, input_tensor, lvl=lvl + 1)
            # Case 1: We have a nested fusion layer -> call fusion method
            elif isinstance(adapter_stack_layer, Fuse):
            ...
            # Case 5: We have a single adapter which is part of this module -> forward pass
                ...
                hidden_states, skip, up = adapter_layer(hidden_states, residual_input=residual)
                ...
                if i == len(adapter_setup) - 1:
                    return hidden_states, up, input_tensor, skip
        ...
        return hidden_states, None, input_tensor, skip

    def adapter_flow(self, adapter_setup: Flow, hidden_states, input_tensor, lvl=0):
        """
        Performs adapter flow with the given adapters for the given input.
        """
        # config of _last_ fused adapter is significant
        adapter_config = self.config.adapters.get(adapter_setup.last())
        # SHAPE: hidden_states: [batch_size, seq_len, d_ml] | query: [batch_size, seq_len, d_ml] | residual: [batch_size, seq_len, d_ml]
        ...
        if len(up_list) > 0:
            # shape: [num_adapters, batch_size, seq_len, d_ml] -> in our context, num_adapters=2 always.
            up_list = torch.stack(up_list)
            # # shape: [batch_size, seq_len, num_adapters, d_ml]
            # up_list = up_list.permute(1, 2, 0, 3)

            hidden_states = self.adapter_flow_layer[adapter_setup.name](
                query,     # shape: [batch_size, seq_len, d_ml]
                up_list,   # shape: [num_adapters, batch_size, seq_len, d_ml]
                up_list,   # shape: [num_adapters, batch_size, seq_len, d_ml]
                residual,  # shape: [batch_size, seq_len, d_ml]
            )
    ...
    def adapters_forward(self, hidden_states, input_tensor, **kwargs):
        ...
        if not skip_adapters and (len(set(self.adapters.keys()) & adapter_setup.flatten()) > 0):
            if isinstance(adapter_setup, Stack):
                hidden_states, _, input_tensor, skip_connections = self.adapter_stack(adapter_setup, hidden_states, input_tensor)
            elif isinstance(adapter_setup, Flow):

                hidden_states = self.adapter_flow(adapter_setup, hidden_states, input_tensor)
