...
@dataclass
class SkipConfig(AdapterConfig):
    """
    Empty adapter that simply passes the input to  output
    """
...
@dataclass
class AdapterFlowConfig(Mapping):
    """Base class that models the architecture of an adapter flow layer."""
    ...
    def load(cls, config: Union[dict, str], **kwargs):
        """
        Loads a given adapter flow configuration specifier into a full AdapterFlowConfig instance.

        Args:
            config (Union[dict, str]): The configuration to load. Can be either:

                - a dictionary representing the full config
                - an identifier string available in ADAPTERFLOW_CONFIG_MAP
                - the path to a file containing a full adapter flow configuration

        Returns:
            dict: The resolved adapter fusion configuration dictionary.
        """
        # currently storing AdapterFlow weights on AdapterHub is not supported.
        config_dict = resolve_adapter_config(config, local_map=ADAPTERFLOW_CONFIG_MAP, try_loading_from_hub=False)
        # convert back to dict to allow attr overrides
        if isinstance(config_dict, AdapterFlowConfig):
            config_dict = config_dict.to_dict()
        config_dict.update(kwargs)
        return AdapterFlowConfig.from_dict(config_dict)

@dataclass
class StaticAdapterFlowConfig(AdapterFlowConfig):
    """
    Static version of adapter flow without a value matrix.
    """
...
@dataclass
class DynamicAdapterFlowConfig(AdapterFlowConfig):
    """
    Dynamic version of adapter flow with a value matrix and regularization. Described in
    """
...
ADAPTERFLOW_CONFIG_MAP = {"static": StaticAdapterFlowConfig(), "dynamic": DynamicAdapterFlowConfig()}


DEFAULT_ADAPTERFUSION_CONFIG = "dynamic"
DEFAULT_ADAPTERFLOW_CONFIG = "dynamic"
