...
class BartEncoderLayerAdaptersMixin:
    ...
    def add_flow_layer(self, adapter_names):
        self.attention_adapters.add_flow_layer(adapter_names)
        self.output_adapters.add_flow_layer(adapter_names)
    ...
    def delete_flow_layer(self, adapter_names):
        self.attention_adapters.delete_flow_layer(adapter_names)
        self.output_adapters.delete_flow_layer(adapter_names)
...
class BartDecoderLayerAdaptersMixin(BartEncoderLayerAdaptersMixin):
    ...
    def add_flow_layer(self, adapter_names):
        super().add_flow_layer(adapter_names)
        self.cross_attention_adapters.add_flow_layer(adapter_names)
    ...
    def delete_flow_layer(self, adapter_names):
        super().delete_flow_layer(adapter_names)
        self.cross_attention_adapters.delete_flow_layer(adapter_names)
...
class BartEncoderDecoderAdaptersMixin:
    ...
    def add_flow_layer(self, adapter_names):
        for layer in self.layers:
            layer.add_flow_layer(adapter_names)
    ...
    def delete_flow_layer(self, adapter_names):
        for layer in self.layers:
            layer.delete_flow_layer(adapter_names)
...
class BartModelAdaptersMixin(ModelAdaptersMixin):
    ...
    def _add_flow_layer(self, adapter_names):
        if hasattr(self, "encoder"):
            self.encoder.add_flow_layer(adapter_names)
        self.decoder.add_flow_layer(adapter_names)
    ...
    def _delete_flow_layer(self, adapter_names):
        if hasattr(self, "encoder"):
            self.encoder.delete_flow_layer(adapter_names)
        self.decoder.delete_flow_layer(adapter_names)
    ...
    def get_flow_regularization_loss(self):
        reg_loss = 0.0
        target = torch.zeros((self.config.hidden_size, self.config.hidden_size)).fill_diagonal_(1.0).to(self.device)
        # encoder
        if hasattr(self, "encoder"):
            for _, v in self.encoder.layers._modules.items():
                for _, layer_flow in v.output_adapters.adapter_flow_layer.items():
                    if hasattr(layer_flow, "value"):
                        reg_loss += 0.01 * (target - layer_flow.value.weight).pow(2).sum()

                for _, layer_flow in v.attention_adapters.adapter_flow_layer.items():
                    if hasattr(layer_flow, "value"):
                        reg_loss += 0.01 * (target - layer_flow.value.weight).pow(2).sum()
        # decoder
        for _, v in self.decoder.layers._modules.items():
