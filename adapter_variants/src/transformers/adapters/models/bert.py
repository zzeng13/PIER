...
class BertLayerAdaptersMixin:
    ...
    def add_flow_layer(self, adapter_names):
        self.attention.output.add_flow_layer(adapter_names)
        self.output.add_flow_layer(adapter_names)
    ...
    def delete_flow_layer(self, adapter_names):
        self.attention.output.delete_flow_layer(adapter_names)
        self.output.delete_flow_layer(adapter_names)

    def enable_adapters(
        self, adapter_setup: AdapterCompositionBlock,
            unfreeze_adapters: bool, unfreeze_attention: bool
    ):
...
class BertModelAdaptersMixin(InvertibleAdaptersMixin, ModelAdaptersMixin):
    ...
    def train_adapter_flow(self, adapter_setup: Union[list, AdapterCompositionBlock], unfreeze_adapters=False):
        ...
        # use the adapters to be trained by default in every forward pass
        self.set_active_adapters(adapter_setup)
        # TODO implement flow for invertible adapters
    ...
    def get_flow_regularization_loss(self):
        ...
        for _, v in self.encoder.layer._modules.items():
            ...
            for _, layer_flow in v.output.adapter_flow_layer.items():
                if hasattr(layer_flow, "value"):
                    reg_loss += 0.01 * (target - layer_flow.value.weight).pow(2).sum()

            for _, layer_flow in v.attention.output.adapter_flow_layer.items():
                if hasattr(layer_flow, "value"):
                    reg_loss += 0.01 * (target - layer_flow.value.weight).pow(2).sum()
        return reg_loss
